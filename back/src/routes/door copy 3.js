...
async function handleDoorByStatus(req, res, session, applyPattern, channelToOff) {
  ...
  if (action === null) {
    const resp = { code: 1001, desc: "invalid status (expect 1=open, 2=close)", data: {} };
    controlLog(`[Door] resp 400 ${session.doorCode}`, resp);
    return res.status(400).json(resp);
  }

  if (action === "close") {
    if (session.currentStatus === 2) {
      const resp = { code: 1000, desc: "ok close (duplicate ignored)", data: {} };
  ...
  if (session.currentStatus === 1) {
    const resp = { code: 1000, desc: "ok open (duplicate ignored)", data: {} };
    controlLog(`[Door] resp 200 ${session.doorCode}`, resp);
    return res.status(200).json(resp);
  }
  try {
...
function logStatusQuery(req, session) {
  const resp = buildStatusBody(session);
  statusLog(
    `[Door] RCS ${req.method} ${req.originalUrl} → resp ${JSON.stringify(resp)}`
  );
  return resp;
}

doorRouter.post("/getstatus1", (req, res) => res.json(logStatusQuery(req, door1)));
doorRouter.get("/getstatus1", (req, res) => res.json(logStatusQuery(req, door1)));
doorRouter.post("/getstatus2", (req, res) => res.json(logStatusQuery(req, door2)));
doorRouter.get("/getstatus2", (req, res) => res.json(logStatusQuery(req, door2)));
doorRouter.post("/getstatus3", (req, res) => res.json(logStatusQuery(req, door3)));
doorRouter.get("/getstatus3", (req, res) => res.json(logStatusQuery(req, door3)));
...
testRouter.post("/clear-all-outputs", async (req, res) => {
  ...
  try {
    ...
    controlLog(`[Door] clear-all-outputs → all sessions reset to status=2`);
    results.door = "ok";
  } catch (e) {
