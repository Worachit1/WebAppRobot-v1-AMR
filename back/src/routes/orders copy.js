...
const { enqueueOrder, getQueueSnapshot } = require("../services/queue");
...
router.post("/", async (req, res) => {
  ...
  console.log(
    `[Orders] enqueue robot=${robot.id} orderId=${orderId} taskPath=${taskPath} deviceNum=${robot.deviceNum} rcsBaseUrl=${rcsBaseUrl || "(empty)"}`
  );
  ...
  res.json({
    ok: true,
    orderId,
    queue: getQueueSnapshot(robot.id)
  });
});
